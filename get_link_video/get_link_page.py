...
header_cookie = ""
page_id = "sofunnysokyoot"
VIDEOS_TXT = page_id + '_videos.txt'
...
next_page_url = f"https://mbasic.facebook.com/{page_id}"
while True:
    page_content = session.get(next_page_url)
    html = bs4.BeautifulSoup(page_content.text, "html.parser")

    next_page_url = "https://mbasic.facebook.com"+html.select("#structured_composer_async_container > div:nth-child(2) >a")[0]['href']
    for article in html.select("[role=article]"):
        if r'video_redirect' in str(article) and "data-ft" in article.attrs:
            ...
            video_url = f"https://www.facebook.com/{post_id}/"
